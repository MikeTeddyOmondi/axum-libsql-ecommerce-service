use crate::db::{all_books, book_by_id, Post};
...
// async fn add_book(
//     Extension(conn): Extension<Connection>,
//     extract::Json(book): extract::Json<Post>,
// ) -> Result<Json<i32>, StatusCode> {
//     if let Ok(new_id) = crate::db::add_book(conn, &book.title, &book.author_id).await {
//         Ok(Json(new_id))
//     } else {
//         Err(StatusCode::SERVICE_UNAVAILABLE)
//     }
...
// async fn delete_book(Extension(conn): Extension<Connection>, Path(id): Path<i32>) -> StatusCode {
...
//         dotenvy::dotenv().ok();
//         let connection = crate::init_db().await.unwrap();
//         let app = crate::router(connection);
//         TestClient::new(app)
//     }
